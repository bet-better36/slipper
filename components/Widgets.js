...
export default function Widgets({newsResults, randomUsersResults}) {
  ...
  return (
    <div className="xl:w-[600px] hidden lg:inline ml-8 space-y-5">
        ...
        <div className="text-gray-700 space-y-3 bg-gray-100 rounded-xl pt-2 w-[90%] xl:[75%] ">
        ...
        <div className="sticky top-16 text-gray-700 space-y-3 bg-gray-100 pt-2 rounded-xl w-[90%] xl:w-[75%]">
          ...
          {randomUsersResults.slice(0, randomUserNum).map((randomUser) => (
            <div key={randomUser.login.username} className="flex items-center px-4 py-2 cursol-pointer hover:bg-gray-200">
          ...
          <button onClick={()=>setRandomUserNum(randomUserNum+3)} className="text-blue-300 pl-4 hover:text-blue-400">Show more</button>
        </div>
    </div>
  );
}
